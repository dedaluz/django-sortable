...
(function($) {
    $(document).ready(function($) {
        ...
        // Hide position input field and show the value as read-only text
        ...
        $(table).find('tbody').sortable({
            ...
            update: function(event, ui) {
                ...
                sort_values = $.map(items, function(item, i) {
                    return parseInt($(item).children(':eq('+pos_col+')').find('input:first').val(), 10);
                });

                //make it work with paged lists
                offset = Math.min.apply( Math, sort_values );
                ...
                $(items).each(function(index) {
                    ...
                    input.attr('value', index+offset);
                    label.text(index+offset);
                });
